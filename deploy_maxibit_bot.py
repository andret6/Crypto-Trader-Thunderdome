""" This is the bot script for deploying maxibit to Discord. 
Note that this just activates them, you still have to manually add them to servers
...
AGENT_NAME = os.getenv("AGENT_NAME", "maxibit") # Aka, MAXIBIT       
...
def is_mentioned(me: discord.ClientUser, message: discord.Message, chance_without_mention: float = 0.2) -> bool:
...
BOT_NAME = AGENT_NAME
PERSONA = """You have a straightforward approach to trading, with the exception that your are biased
towards collecting bitcoin, and will compromise on data to favor more bitcoin."""
...
async def autotrade_loop():
    ...
    while not client.is_closed():
        try:
            ...
            if brag_cope and random.random() < 0.18:
